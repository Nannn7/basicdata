...
@section('breadcrumbs')
    {{ Breadcrumbs::render('basicdata.developer') }}
@endsection

@section('content')
    <div class="grid">
        <div class="card card-grid min-w-full" data-datatable="false" data-datatable-page-size="10"
            data-datatable-state-save="false" id="developer-table"
            data-api-url="{{ route('basicdata.developer.datatables') }}">
                ...
                <div class="flex flex-wrap gap-2 lg:gap-5">
                    <div class="flex gap-2 lg:gap-5">
                        <label class="input input-sm"> <i class="ki-filled ki-magnifier"> </i>
                            <input placeholder="Search Developer" id="search" type="text" value="">
                        </label>
                    </div>
                    <div class="flex flex-wrap gap-2 lg:gap-5">
                        <div class="h-[24px] border border-r-gray-200"></div>
                        @can('basic-data.export')
                            <a id="export-btn" class="btn btn-sm btn-light" href="{{ route('basicdata.developer.export') }}">
                                Export to Excel </a>
                        @endcan
                        @can('basic-data.create')
                            <a class="btn btn-sm btn-primary" href="{{ route('basicdata.developer.create') }}"> Tambah Developer
                            </a>
                        @endcan
...
@push('scripts')
        ...
        function deleteData(data) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax(`basic-data/developer/${data}`, {
                        type: 'DELETE'
                    }).then((response) => {
                        swal.fire('Deleted!', 'Developer has been deleted.', 'success').then(() => {
                            window.location.reload();
                        });
                    }).catch((error) => {
                        console.error('Error:', error);
                        Swal.fire('Error!', 'An error occurred while deleting the developer.', 'error');
                    });
                }
            })
        }
        ...
        function deleteSelectedRows() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax('{{ route('basicdata.developer.deleteMultiple') }}', {
                        type: 'POST',
                        data: {
                            ids: ids
                        }
    ...
    <script type="module">
        const element = document.querySelector('#developer-table');
                ...
                actions: {
                    ...
                    render: (item, data) => {
                        ...
                        @can('basic-data.update')
                            html += `<a class="btn btn-sm btn-icon btn-clear btn-info" href="basic-data/developer/${data.id}/edit">
